...
Default to the inherited parent model unless the user requests a specific one.

Pi can use any model shown by `pi --list-models`. Prefer `provider/model-id`; a bare model id only works when unambiguous. If the user names a model, look it up in `pi --list-models` and use its exact `provider/model-id`.
...
**Best default:** inherit the parent model and reasoning level. If the user names a model, use it.

| Model hint | Model               | Recommended effort |
| ---------- | ------------------- | ------------------ |
| (none)     | inherited default   | `high`             |
...
**Best default:** inherit the parent model with `high` effort unless the user specifies otherwise.

| Model         | Recommended effort |
| ------------- | ------------------ |
| user-specified | per request       |
